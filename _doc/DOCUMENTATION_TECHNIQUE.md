...
L'application est ensuite accessible depuis les endpoints suivants :
...
Le projet présente une base DevOps solide, mais plusieurs axes d’amélioration sont prioritaires pour atteindre un niveau de qualité et de sécurité optimal.

La couverture de tests, encore insuffisante côté back et front, doit être renforcée pour fiabiliser les évolutions et réduire les risques de régression.

Par ailleurs, la gestion des vulnérabilités nécessite d’être durcie, notamment en étendant le seuil de blocage Trivy aux failles HIGH et en maintenant les dépendances à jour.

Des améliorations sont également attendues sur la qualité du code, avec une réduction de la duplication et la correction des hotspots de sécurité (notamment CORS) afin de valider la Quality Gate.

Enfin, certaines bonnes pratiques DevOps restent à généraliser, comme l’exécution en non-root pour tous les conteneurs, l’automatisation de la gestion des dépendances et la mise en place d’un monitoring en production, afin de rendre le système plus robuste, sécurisé et maintenable à long terme.
